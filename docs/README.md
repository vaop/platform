# VAOP Documentation

Welcome to the Virtual Airline Operations Platform documentation.

## Getting Started

- [Quick Start](getting-started/quick-start.md) - Get running in minutes
- [Installation](getting-started/installation.md) - Shared hosting and VPS setup
- [Requirements](getting-started/requirements.md) - System requirements

## Configuration

- [Environment Variables](configuration/environment.md) - Settings reference

## Admin Guide

- [Registration Settings](admin/registration-settings.md) - User registration and approval

## Deployment

- [Upgrading](deployment/upgrading.md) - How to update VAOP
- [Backups](deployment/backups.md) - Backup and restore your data
- [Docker](deployment/docker.md) - Container deployment (advanced)
